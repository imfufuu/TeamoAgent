...
export const SUGGESTIONS = [
  // 代码沙箱
  { text: '用沙箱计算：前 100 个斐波那契数中有多少个质数？', tag: 'JS 沙箱' },
  { text: '写一段 JS 在沙箱里用蒙特卡洛估算 π，跑 10 万次并验证误差', tag: 'JS 沙箱' },
  { text: '用 Python 沙箱（Pyodide）画出 1~20 的阶乘增长表，并把结果写入 files/factorials.md', tag: 'Python 沙箱' },
  { text: '在 C++ 沙箱里写一个快速排序，用 10 组随机数组对拍 std::sort 验证正确性', tag: 'C++ 沙箱' },
  { text: '把《静夜思》写入 files/poem.txt，读出来后逐句翻译成英文并存成 files/poem.en.md', tag: '虚拟文件' },
  // 生图 / 图片编辑
  { text: '画一张「深夜实验室里的机械猫」，1024x1024，生成后把图写进沙箱并告诉我路径', tag: '文生图' },
  { text: '用 generate_image 生成三张候选图（n=3），再把最满意的一张改成夜晚霓虹配色', tag: '文生图 · 改图' },
  { text: '把我刚上传的图片改成透明底，并把结果连同原图一起打包成 ZIP', tag: '图片编辑' },
  // 文件 / 导出
  { text: '在沙箱里建三个目录 demo/a、demo/b、demo/c 并各写一个文件，然后按目录树列出来', tag: '目录树' },
  { text: '生成一份 200 行的销售数据 CSV 写入沙箱，用 JS 聚合出 Top5 并输出 Markdown 表格', tag: '数据处理' },
  // 子智能体 / 工具循环
  ...
  // 上下文 / 回滚
  { text: '连续问我三轮再回答，最后演示用「回滚」退回到第二轮之前，并说明消息数组怎么变的', tag: '对话回滚' },
  { text: '把一段 3000 字的中文长文压缩成 5 条要点，再展开成结构化大纲（验证上下文压缩策略）', tag: '上下文' },
  { text: '解释一下 TeamoRouter 的 Claude 为什么要走 /v1/messages 而不是 /v1/chat/completions，并给出请求头示例', tag: '协议' },
];
