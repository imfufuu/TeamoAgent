...
// 卡片展示约 26–38 字概括（title，略长以免空状态三条宽窄不一）；点击填入约 200 字的完整提示词（text）。
...
export const SUGGESTIONS = [
  {
    title: '用 Python 做线性回归并写出带残差表的完整报告',
  ...
  {
    title: '在 C++ 沙箱实现 LRU 缓存并和暴力解对拍一百组',
  ...
  {
    title: '生成 TeamoAgent 发布海报并写入沙箱路径保存',
  ...
  {
    title: '在沙箱搭三页静态站并打包成可分发的 ZIP 文件',
  ...
  {
    title: 'NFC 与 NFD 往返日文浊音并解释码位差异细节',
  ...
  {
    title: '在沙箱写 JSON Schema 校验器并跑正反用例落盘',
  ...
  {
    title: '解码一段 JWT 并核对 header 里的算法字段记录',
  ...
  {
    title: '对比两份配置并用 diff 生成可应用的补丁文件',
  ...
  {
    title: '写一个 Markdown 看板并导入三张带标签的卡片',
