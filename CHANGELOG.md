...
## Teamo V1.2 正式版（2026-09-26）

**当前线上版本即 V1.2 正式版**，构建号 `2026.9.26.38`。

* 沙箱文件可用 ![alt](sandbox://path) 嵌进正文；生图不再出现在工具芯片。选择框 / 折叠栏 / 文内目录锚点。Edited file(s) 按数量变单复数。Ultra 彩虹略加快。思考与正文行距略增。

* 「思考过程」四字改灰；对话链接改为蓝色下划线（用户气泡仍反色）。

* README 增加 TL;DR，开头先讲能做什么；对话行距收紧，思考正文与标题对齐不再顶到左边。

* CTA 弧线蒙在收尾区块内，不再画进上方 FAQ；思考过程与工具芯片同构展开；含工具的气泡去掉脚注并收紧芯片行距；所有工具完成都标耗时。

* 身份钉死 TeamoAgent（禁止自称 Kiro）；token 详情改为小浮窗；Jev 与复制同行；思考过程整行可点；介绍页加入思考档对照表。

* Ultra 相对 High：32k 思考预算、更大输出帽、可委派，并强制自检与多专家交叉复核。

* 气泡脚注与复制/回滚/重新生成同一行且右对齐；Claude 5 / GPT / Gemini 无可见思考正文时标明「已思考」。

* 思考 Off 不再展示思考过程；每条回复记下思考等级，气泡脚注 `1m 50s | 2 minutes ago`。

* 收尾「现在就开始」改成绕标题的正圆短弧，不再把宽矩形边框拧成斜线划过文字。

* 模型菜单本地过滤 glm-5.3-flash-free（网关仍返回也不显示）；代码块补主流语言高亮。

* 下线已不可用的 glm-5.3-flash-free。

* 任务卡标题加长且长短不一；模型热门/低价按 2026-09 调研重打。

* 空状态任务卡拉齐宽度并加长标题；新增 remember 跨会话长效记忆。

* 联系方式只出现在介绍页，对话页不再展示。

* 页脚联系邮箱与 GitHub；提示词写明作者 imfufuu。

* 「为什么选我们？」卡片改为「本地密钥」。

* 收尾标题改慢转弧线；会话改名只留一层边框；打开模型菜单不再抢焦点。

* 门页文案改为「正在加载影片 / 影片已就绪」；「现在就开始」去句号并加动效。

* 门页整段缓冲配乐，进度条走完再开片，避免边下边播卡顿。

* 介绍页进站停在首屏，不再落到「为什么选我们？」；产品版本升至 V1.2。

## Teamo V1.1 正式版（2026-09-26）

构建号至 `2026.9.26.18`。

* 导航「为什么选我们？」问号；字幕「智能体」去句号；终端三点改红黄绿。

* 导航页：Local Key、原则与 FAQ 拉开行距、补「为什么选我们」。

* 落地门页：轨道动效、片头说明、快捷键与跳过入口，不再只有冷冰冰一行字。

* 安全：Markdown 剥离 javascript:/data: HTML 链接；中继校验 Origin、限制代理路径、安全响应头。

* 文档对齐现状；对话页字标减细；外观用颜色插值。有中继才开联网。

* 联网：有本地中继才可开；无中继按钮始终灰色不可点。

* 字幕改成整句；外观用颜色插值反转、去掉闪白；导航页补工作台与原则；字标减细。
* 导航页下滚时元素依次滑入。
* 抛光只打在当前焦点镜头上。
* 介绍片字幕改到底部字幕条并缩小，不再挡住镜头。
* 片尾最后 5 秒先渐黑再渐白后进导航页；导航图标与对话页同一套线性 SVG；「开始探索」补上品牌标。
* 产品介绍主页（`index.html`）可跳进对话页 `app.html`。
* 空状态任务示例：卡片 20–30 字概括，点击填入约 200 字提示。
* 本地工作台：`search_files` / `diff_text` / `json_tool` / `delete_file` / `copy_file`。
* 沙箱侧栏字号与会话列表对齐。

* 侧栏 Logo 旁的 `V1.1` 徽章；底部「**Teamo V1.1 正式版** · v构建号」。
* 思考档 Ultra：斜向丝绸彩虹（旋转无缝色带，粉紫交界不再切竖缝、滑动中不再闪白）。
* 沙箱面板改成 Workspace 式卡片：标题 + 容量/文件数 + 图标下载/清空，文件树无行框。
* 子智能体名录从面板拿掉（委派仍仅 Max/Ultra）；任务示例改为短主题、点击填完整提示词。
* 回滚每轮只出现在该轮最后一条助手消息上。

## Teamo V1.0 正式版（2026-09-22）

V1.0 于 2026-09-22 发布，构建号 `2026.09.22.1`。版本在当时界面上有四处可见/可查：

* 侧栏 Logo 旁的 `V1.0` 徽章（`.ver-badge`）；
* 侧栏底部「**Teamo V1.0 正式版** · v2026.09.22.1」（`#build-stamp`，仍会比对入口 meta 检测缓存错配）；
* `<meta name="app-release" content="V1.0">` 与 `<meta name="app-version" content="2026.09.22.1">`；
* 页面标题「TeamoAgent V1.0 正式版 — 网页端智能体」，以及控制台横幅。

代码里 `APP_RELEASE`（发布版本）与 `APP_VERSION`（构建戳，用于 `?v=` 缓存击穿）分开维护：前者只在正式发版时变。

### V1.0 覆盖的能力（均已上线并实测）
模型自选（41 个网关模型 + 家族图标 + 搜索）· 全模型思考模式与自动降级 · 代码沙箱（JS/Python/C++
+ 虚拟文件系统 + 单文件/整包下载）· 文生图与图片编辑（由主 Agent 调用）· 多会话记录（导出/导入 JSON）·
对话回滚与撤销 · 覆盖式「重新生成」（只给最近一条）· 附件自动入沙箱 `uploads/` · KaTeX 公式 ·
**模型 API 自带的原生联网检索**（Claude / GPT，含来源条与诚实性护栏）· 18 个子智能体委派 ·
输出用时与 token 统计 · 移动端独立排布层 · 本地中继（`fetch_url` / `run_git`，静态托管时自动从工具表移除）·
刷新后不丢数据（重数据存 IndexedDB）。

### 发版后的移动端审计补漏
线上复测抓到两个触控目标偏小：主题按钮（去掉文字后只有 33px 宽）与文件行的下载键 34px 宽，
都低于「≥36px」的标准。触屏层给「只有图标没有文字的按钮」统一成 40×40 方框后，320/360/390/414/768
五个视口全部通过；`npm run test:touch` 里的尺寸断言也从 ≥32px 提到 ≥36px，与审计同一标准。

### 本次随发版的仓库清理
* 删除我调试期间误入库的探针脚本 `.mq.mjs`；
* 删除 `__pycache__/`（本就在 `.gitignore` 里，顺手清掉磁盘残留）；
* 删除已过期的内部评估报告 `ANALYSIS.md`（27KB，结论早已落到 CHANGELOG 各节；需要回溯可看 git 历史），
  并修正 README / CHANGELOG 里对它的引用；
* 复核资产无孤儿：7 个厂商图标与 20 个 KaTeX 字体文件全部被引用，20 个 `js/*.js` 全部有引用方，
  `tests/` 下 11 个脚本都是回归或线上检查用，全部保留。
...
（`@media (hover: none)`，headless 与带触摸屏的 Windows 笔记本都会命中）。量出来的实际数字：

| 位置 | 修前 | 修后 |
| --- | --- | --- |
| 模型选择菜单每行 | `min-height: 44px` → 行高 44px | `min-height: 38px` → 行高 38px |
| 左下角四个按钮 | 字号/内边距一起放大 + 中文标签换行 → `66×58 / 49×58 / 49×58 / 53×58`（两行文字撑成 58px 高） | `white-space: nowrap` + 触屏层只抬 min-height → 单行 40px，四个等高同排 |
| 行距观感 | 菜单一屏只放下 9 行 | 12+ 行 |

修法：触屏层只保证「够点得中」（min-height），不再顺手放大字号与内边距；`white-space: nowrap` 收进
`.mini-btn` 基础样式，任何按钮的文字都不再折行。新增 `tests/touch-density.mjs`（真 Chromium，
headless 默认就是 hover:none，正好复现用户环境）在 1280×900 与 390×844 两个视口量计算样式，
10 项断言覆盖这些数字。

### 1. 工具调用图标改 SVG，跑完停转
芯片图标原先是 `⚙` 字符且 `animation: spin 3.5s linear infinite` 永久旋转。现在是线性扳手 SVG
（`ICON.tool`），只在**未完成**时转（`.chip:not(.done) .chip-ico`），工具结果回填时打上 `.done`
→ 图标定格，状态位同时给出 ✓/✕。

### 2. 刷新模型按钮只转箭头
以前把 `.spin` 加在按钮本体上（边框、背景一起转）。改成 `.icon-btn.spin svg` 只转箭头；
顺带把按钮里的图形字符 `⟳` / `⟨` / `☰` / `✕` 换成同一套线性 SVG（用户定的图例：SVG + 中文，
不用字符图形）—— 换句话说，以前那个按钮里根本没有「箭头」可转。

### 3. 没有会话记录时的提示改短
侧栏空列表原来写「还没有会话记录。直接输入第一条消息，它才会出现在这里。」（两行），
现在就是一句 **「还没有会话记录」**。

### 4. 再次打开网页时，对话 / 附件 / 沙箱都还在（根治配额问题）
真机复测发现：文本内容与文本附件刷新后是能恢复的，**但图片会丢**。原因是附件图片、沙箱里的
data URL、生成图全是 base64，统一塞在 localStorage（约 5MB）里，一旦超限 `setItem` 抛
`QuotaExceededError`，旧代码会**静默**退化成「瘦身版」快照（剥掉所有 dataUrl）—— 用户看到的就是
「刷新后附件和沙箱没了」。现在：

* 新增 `js/blobstore.js`：小封装把重数据按 key 存进 **IndexedDB**（配额按磁盘算，通常几百 MB）；
* `js/state.js` 新增 `extractBlobs()` / `applyBlobs()` / `collectBlobKeys()` 三个纯函数：
  超过 16KB 的 sandbox 文件与 data URL、超过 2 万字符的附件文本外置出去，localStorage 里只留
  轻量状态 + 一份索引；写入失败会退回旧路径并 warn，不会静默丢数据；
* 启动时 `store.hydrateBlobs()` 后台取回，取到后 `ui.afterHydrate()` 重绘消息与文件面板
  （生成图也因此能在刷新后重新显示：图现在也记在工具调用记录上）；
* 会话/消息删掉后 `blobPrune()` 清掉孤儿数据，不会越积越多。

新增 `tests/persist.mjs`（真 Chromium + 真 IndexedDB，`npm run test:persist`）：发一条带
263KB 图片与 12 万字符文本附件的消息 → 刷新 → 10 项断言（消息、会话、附件图有数据、沙箱图与文本、
IDB 条数、localStorage < 100KB 且无 base64、索引齐全、零错误零告警）全部通过。
...
测试：单测 159 → **161**（新增重数据外置与孤儿清理）、dom-smoke 156 → **166**、
app-boot 82 → **85**（新增芯片 SVG/停转、空会话短提示）；另新增两条真 Chromium 测试：`tests/persist.mjs`（刷新持久化 10 项）与
`tests/touch-density.mjs`（触屏密度 20 项，两个视口）。

历史计数：单测 156 → **159**（新增 webRefusal 正反例、提示词漂移守卫、无中继时工具表收敛）、
dom-smoke 156（不变）、app-boot 66 → **82**（新增「开关开着却回拒答」提示条分组、对照组、
以及重试按钮的改写/覆盖/来源条回归）、server_checks 51（不变）。
`APP_VERSION` / `?v=` / `<meta name="app-version">` 全部 → `2026.09.21.10`。

### 1d. 提示条再进一步：一键「重试并联网检索」

1c 的提示条只是「告诉你怎么办」，用户还得自己把问题重打一遍。上线后又跑了几轮真机对照：
同一句提问**加上「先联网检索再回答」前缀**后命中率明显变好（实测 3 次里 1 次真检索，成功那次拿到
10 条来源、6 条可点链接）。于是提示条上直接挂一枚按钮（SVG 图标 + 中文，与消息操作条同一套视觉语言）：
点一下就**覆盖式重试** —— 先抹掉那条没检索到的回答，把原提问改写成「先联网检索再回答：…」再重发，
既不留下两条并列、也不让用户重打。已经写过前缀的再点会提示换模型（Claude / GPT 系列），
非最近一轮则提示先「回滚」。

顺带修掉一个真 bug：提示条是在消息动作条的监听器绑定**之后**才挂上去的，第一版把重试按钮混进
通用 `.act` 处理器里点了没反应（app-boot 立刻抓到了），现在改成具名 `doWebRetry()` 直接绑定。
