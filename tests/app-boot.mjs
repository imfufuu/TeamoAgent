...
await tick(30);
ok('侧栏底部写明「Teamo V1.2 正式版」+ 构建号', /Teamo V1\.2 正式版/.test($('#build-stamp').textContent) && /v\d{4}\.\d{1,2}\.\d{1,2}\.\d+/.test($('#build-stamp').textContent),
  $('#build-stamp').textContent.trim());
ok('侧栏 Logo 旁 V1.2 徽章在界面上', !!$('.ver-badge') && $('.ver-badge').textContent.trim() === 'V1.2');
