...
console.log('\n⑯ 移动端布局：根因修复 + 密度重排（源码级护栏）');
{
  ...
  // V1.2 正式版标识：标题 / meta / 侧栏徽章 / 底部版本戳都要写出来
  const cfgSrc = fs.readFileSync(path.join(ROOT, 'js/config.js'), 'utf8');
  ok('config.js 分开维护发布版本与构建戳', /APP_RELEASE = 'V1\.2'/.test(cfgSrc) && /APP_VERSION = '/.test(cfgSrc));
  ok('标题与 meta 标明 V1.2 正式版', /<title>[^<]*V1\.2 正式版[^<]*<\/title>/.test(htmlSrc) && /<meta name="app-release" content="V1\.2"/.test(htmlSrc));
  ok('侧栏 Logo 旁有 V1.2 徽章', /class="ver-badge"[^>]*>V1\.2</.test(htmlSrc) && /\.ver-badge\s*\{/.test(cssText));
  ok('底部版本戳用 APP_RELEASE 写明正式版', /Teamo \$\{APP_RELEASE\} 正式版/.test(uiSrc) && /APP_RELEASE\b/.test(uiSrc));
  ok('控制台横幅也是 V1.2', /TeamoAgent V1\.2 正式版/.test(fs.readFileSync(path.join(ROOT, 'js/main.js'), 'utf8')));
